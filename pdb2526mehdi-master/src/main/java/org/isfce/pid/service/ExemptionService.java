...
public class ExemptionService {
    ...
    public ExemptionRequestFullDto addManualItem(UUID requestId, AddManualExemptionItemDto dto) {
        ...
        // Important : On ajoute l'item à la liste du parent en mémoire 
        // pour que le mapper le voie immédiatement dans le retour.
    ...
    public ExemptionRequestFullDto deleteItem(UUID itemId) {
        ...
        // Sécurité : On ne touche pas si c'est déjà soumis
        if (req.getStatut() != StatutDemande.DRAFT) {
            throw new IllegalStateException("Impossible de supprimer : dossier verrouillé");
        }
        
        // Suppression propre
        // 1. On l'enlève de la liste du parent en mémoire (important pour le retour immédiat)
        req.getItems().remove(item);
        // 2. On supprime en base
        itemDao.delete(item);
        
        // On renvoie le dossier à jour
        return mapper.toExemptionRequestFullDto(reqDao.save(req));
    }
}